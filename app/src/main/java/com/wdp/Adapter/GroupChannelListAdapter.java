...
import com.wdp.SendBird.DateUtils;
import com.wdp.SendBird.FileUtils;
import com.wdp.SendBird.TextUtils;
import com.wdp.SendBird.TypingIndicator;
...
public class GroupChannelListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    private List<GroupChannel> mTempChannelList;
    ...
    public GroupChannelListAdapter(Context context) {
        ...
        mTempChannelList = new ArrayList<>();
    }
    ...
    public void load() {
        try {
            ...
            for (int i = 0; i < dataArray.length; i++) {
                ...
                mTempChannelList.add((GroupChannel)BaseChannel.buildFromSerializedData(Base64.decode(dataArray[i], Base64.DEFAULT | Base64.NO_WRAP)));
            }
    ...
    public void setGroupChannelList(List<GroupChannel> channelList) {
        ...
        mTempChannelList = channelList;
        notifyDataSetChanged();
    }

    public void addLast(GroupChannel channel) {
        mChannelList.add(channel);
        mTempChannelList.add(channel);
    ...
    public void filter(String charText) {
        ...
        if (charText.length() == 0) {
            mChannelList.addAll(mTempChannelList);
        } else {
            for (int i=0; i<mTempChannelList.size(); i++){
                if (mTempChannelList.get(i).getMembers().get(0).getNickname().equalsIgnoreCase(SendBird.getCurrentUser().getNickname())){
                    if (mTempChannelList.get(i).getMembers().size()>1){
                        if (mTempChannelList.get(i).getMembers().get(1).getNickname().toLowerCase(Locale.getDefault()).contains(charText)){
                            mChannelList.add(mTempChannelList.get(i));
                        }
                    }

                }else {
                    if (mTempChannelList.get(i).getMembers().get(0).getNickname().toLowerCase(Locale.getDefault()).contains(charText)){
                        mChannelList.add(mTempChannelList.get(i));
                    }
                }
